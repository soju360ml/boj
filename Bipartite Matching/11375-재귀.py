...
def dfs(인접리스트, 시작사람, 제외일자리리스트, 직원키일자리리스트, 일자리키직원값리스트):
        ...
        (반복) 인접리스트에서 시작사람의 일자리 리스트를 꺼낸다:
            ...
            2이미 일자리키직원값리스트에 포함되어있는가?:
                ...
                제외일자리리스트에 현재 노드-피노드 대입
                결과 = dfs(인접리스트, 누구, 제외일자리리스트, 직원키일자리리스트, 일자리키직원값리스트)
                3결과가 참인가?
                    # 재귀루프 탈출 후 결과가 참이면 해당 일자리는 탈취가 가능하므로 리턴!
            ...
            5이미 매칭된 일자리가 아닌가?
                ...
                리턴 참
        (반복문끝) -> 매칭 가능한 일자리 없음
        리턴 거짓   # 증가경로를 찾지 못했으므로 해당 노드는 어느 노드와도 매칭이 불가함
